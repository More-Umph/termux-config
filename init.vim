...
"
let string = {}
    fun! string.matches(src, pattern) dict
        return a:src =~ a:pattern
        endf

    fun! string.reverse(src)
        return join(reverse(split(a:src, '\zs')), '')
        endf


let cursor = {}
    fun! cursor.pos(row, col) dict
    ...
    fun! cursor.column() dict
        return col('.')
        endf

    fun! cursor.line() dict
        return getline(self.row())
        endf
